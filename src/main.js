...
class AcodePlugin {
	constructor() {
		...
		this._codeChangeTimer = null; // debounce do ImportIntellisense (ver buildImportIntellisensePlugin)
...
.cm-tooltip.cm-completionInfo {
  ...
  border-radius: 2px;
	...
	// Antes rodava handleCodeChange() (que consulta o fileIndex de forma
	// assíncrona) a cada tecla digitada em qualquer lugar depois de um "<"
	// na linha — debounçado aqui pra não disparar query em rajada durante
	// digitação normal.
			...
			class {
				update(update) {
					if (update.docChanged) {
						clearTimeout(self._codeChangeTimer);
						self._codeChangeTimer = setTimeout(() => {
	...
	async destroy() {
		clearTimeout(this._codeChangeTimer);
		if (this.editor) {
